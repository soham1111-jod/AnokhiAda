...
//     country?: string;
//   };
//   // Or flat address fields for backward compatibility
//   street?: string;
//   city?: string;
//   state?: string;
//   pinCode?: string;
...
//     if (editData.phoneNo && editData.phoneNo.length !== 10) {
//       toast({
//         title: "Validation Error", 
...
//   // Fetch complete user profile with enhanced debugging
//   const fetchUserProfile = useCallback(async () => {
//     if (!user) return;
    
//     setLoading(true);
//     try {
//       console.log('🔄 Fetching user profile for:', user._id);
...
//       console.log('📱 Phone verification status:', profileData.isPhoneVerified);
//       console.log('🏠 Address data:', profileData.address);
      
//       setUserProfile(profileData);
      
//       // Initialize edit data with better validation
//       setEditData({
...
//       // Enhanced fallback with auth context
//       if (user) {
//         console.log('⚠️ Using fallback data from auth context:', user);
//         setUserProfile(user);
//         setEditData({
...
//         description: error.response?.data?.message || "Some profile data may not be available. Please try refreshing.",
//         variant: "default"
//       });
//     } finally {
//       setLoading(false);
...
//   // Handle profile update with improved error handling
...
//         phoneNo: editData.phoneNo ? editData.phoneNo.trim() : null, // Keep as string
//         address: {
...
const Profile = () => {
  ...
  // Enhanced utility function to format phone number for display
  const formatPhoneNumber = (phoneNo: string | number | undefined): string => {
    if (!phoneNo) return 'Not provided';
    
    const phoneStr = phoneNo.toString();
    
    // If already properly formatted with +91 and space
    if (phoneStr.startsWith('+91 ')) {
      return phoneStr;
    }
    
    // If it starts with 91 (like 918590863462), format it properly
    if (phoneStr.startsWith('91') && phoneStr.length === 12) {
      const actualNumber = phoneStr.substring(2); // Remove '91' prefix
      return `+91 ${actualNumber.replace(/(\d{5})(\d{5})/, '$1-$2')}`;
    }
    
    // If it's just 10 digits (like 8590863462)
  ...
  const fetchUserProfile = useCallback(async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get('/auth/user/profile'); // Fixed endpoint
      const profileData = response.data.user || response.data;
      ...
      // Clean and format phone number for editing
      let cleanPhoneForEdit = '';
      if (profileData.phoneNo) {
        const phoneStr = profileData.phoneNo.toString();
        
        // Extract just the 10-digit number for editing
        if (phoneStr.startsWith('+91 ')) {
          cleanPhoneForEdit = phoneStr.substring(4); // Remove '+91 '
        } else if (phoneStr.startsWith('91') && phoneStr.length === 12) {
          cleanPhoneForEdit = phoneStr.substring(2); // Remove '91' prefix
      ...
      setEditData({
        firstName: profileData.firstName || '',
        lastName: profileData.lastName || '',
        phoneNo: cleanPhoneForEdit, // Just the 10-digit number for editing
        street: profileData.address?.street || profileData.street || '',
        city: profileData.address?.city || profileData.city || '',
        state: profileData.address?.state || profileData.state || '',
        pinCode: profileData.address?.pinCode || profileData.pinCode || '',
      ...
      if (user) {
        ...
        // Handle phone number for fallback data too
        let cleanPhoneForEdit = '';
        if (user.phoneNo) {
  ...
  // Handle profile update with enhanced phone formatting
  const handleSaveProfile = async () => {
    if (!userProfile || !validateEditData()) return;
    
    setIsSaving(true);
    try {
      // Clean the phone number first (remove any non-digits)
      const cleanPhoneNumber = editData.phoneNo ? editData.phoneNo.replace(/\D/g, '') : null;
      
      const updatePayload = {
        firstName: editData.firstName.trim(),
        lastName: editData.lastName.trim(),
        phoneNo: cleanPhoneNumber ? `+91 ${cleanPhoneNumber}` : null, // Always format with +91 and space
        address: {
          street: editData.street.trim() || null,
          city: editData.city.trim() || null,
          state: editData.state.trim() || null,
      ...
      const response = await axiosInstance.put('/auth/user/profile', updatePayload); // Fixed endpoint
      const updatedProfile = response.data.user || response.data;
